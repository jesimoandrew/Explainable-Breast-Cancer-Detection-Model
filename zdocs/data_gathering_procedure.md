...
**1.4 Deployability check.** For each architecture, the single-input student's per-image predictions are compared against its dual-input teacher's on the identical test split using **McNemar's test**, quantifying how much accuracy is surrendered by dropping the expert crop at inference. This establishes whether the deployable formulation is statistically distinguishable from its own upper bound.
...
**3.6 Statistical comparison of architectures.** Four architectures give six pairwise comparisons. The continuous, long-tailed IoU and Dice distributions are compared with the **Wilcoxon signed-rank test**; the paired binary Pointing Game outcome is compared with **McNemar's exact test**, which is the correct instrument for a 0/1 paired column. All six p-values receive **Holm–Bonferroni correction** to control the family-wise error rate. Interpretability rank is then cross-tabulated against the classification rank from Experiment 1, to test whether the most accurate architecture is also the most interpretable.
...
| 1 — Baseline comparison | Accuracy, Sensitivity, Specificity, F1, AUC-ROC, training time per architecture; cached test-set probabilities | 4 training runs, identical recipe; 319-image test set | Bootstrap 95% CI (2,000 resamples); McNemar vs. dual-input teacher |
